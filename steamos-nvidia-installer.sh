...
# Notes: By default nvidia-open = RTX 20xx+ (Turing) only. Pass --proprietary
# to install the proprietary nvidia-dkms driver instead, which supports older
# cards such as the GTX 10xx (Pascal) family — e.g. a GTX 1050. Target machines
# need UEFI + Secure Boot off. First boot of an installed system lands in the
# gamescope Steam setup; if it black-screens: Ctrl+Alt+F3 → steamos-session-select plasma.
...
TRIM_CUDA=0
SKIP_SIG=0
DRIVER_MODE=open        # open (nvidia-open, RTX 20xx+) | proprietary (nvidia-dkms, GTX 10xx/Pascal+)
WORKDIR=""
IMG=""
...
# all nvidia packages must be the same driver release (mirror mid-bump
# skew would make pacman -U fail on the versioned dependency)
DKMS_PKG=$([[ $OPEN_DRIVER -eq 1 ]] && echo nvidia-open-dkms || echo nvidia-dkms)
for f in "${PKG_FILES[@]}"; do
  case "$f" in
    ${DKMS_PKG}-*|lib32-nvidia-utils-*)
      [[ "$f" == *"$NV_PKGVER"* ]] || die "Version skew between nvidia packages (mirror mid-update?) — retry in an hour" ;;
  esac
done
if [[ $OPEN_DRIVER -eq 1 ]]; then
  log "Driver pinned: nvidia-open $DRIVER_VERSION"
else
  log "Driver pinned: nvidia (proprietary) $DRIVER_VERSION"
fi
...
if [[ $OPEN_DRIVER -eq 1 ]]; then
  ...
  compgen -G "$MNT/usr/lib/firmware/nvidia/*/gsp_*.bin" >/dev/null \
    || log "GSP firmware not present — OK for proprietary driver on Pascal (native mode)"
fi
...
cat <<EOF
  ...
  Driver:  $([[ $OPEN_DRIVER -eq 1 ]] && echo "nvidia-open (DKMS)" || echo "nvidia (proprietary DKMS)") $NVIDIA_VER for kernel $KVER
           (latest Arch at build time, pinned — Valve's mirror only has 575.x)
           DRIVER_MODE=$([[ $OPEN_DRIVER -eq 1 ]] && echo open || echo proprietary)
$( case $UPDATE_MODE in
